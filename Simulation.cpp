...
Simulation::Simulation(string file_name) {
	...
	// current time is -1 because it get incremented at the start of the step(),
	// which is called before the print() function. 
	current_time = -1;
...
bool Simulation::done() {
	// The method returns true if there is no customer arriving, in queue,
...
void Simulation::step() {
	...
	// this is to avoid wrong calculations with the vehicle list size changing 
	...
	for (int i = 0; i < tempSize; ++i) {
		if (vehicles_list->get(0).get_arrival_time() == current_time) {
			...
			// because the list is sorted, all the items arriving at the right time
